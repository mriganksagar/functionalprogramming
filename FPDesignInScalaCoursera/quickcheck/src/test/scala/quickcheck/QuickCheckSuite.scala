...
class QuickCheckSuite extends munit.FunSuite:
  ...
  /** Turns a `Properties` instance into a single `Prop` by combining all the
    * properties
    */
  def asProp(properties: Properties): Prop =
    Prop.all(properties.properties.map(_._2).toSeq*)

  test("Binomial heap satisfies properties. (5pts)") {
    assert(
      check(asProp(new QuickCheckHeap with quickcheck.test.BinomialHeap))(
        identity
      ).passed
    )
  }
