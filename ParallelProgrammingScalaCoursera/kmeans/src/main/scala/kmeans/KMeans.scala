...
class KMeans extends KMeansInterface:
  ...
  def converged(eta: Double, oldMeans: ParSeq[Point], newMeans: ParSeq[Point]): Boolean =
    oldMeans.zip(newMeans).forall(means => means._1.squareDistance(means._2) < eta)
  ...
  final def kMeans(points: ParSeq[Point], means: ParSeq[Point], eta: Double): ParSeq[Point] =
    ...
    if !converged(eta, means, newMeans)
      then kMeans(points, newMeans, eta) else means // your implementation need to be tail recursive
